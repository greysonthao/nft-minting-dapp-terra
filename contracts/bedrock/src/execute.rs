use cosmwasm_std::{BankMsg, Coin, Deps, DepsMut, Empty, Env, MessageInfo, Response, StdError};

use crate::msg::MintMsg;
use crate::state::Extension;
use cw721_base::state::{Cw721Contract, TokenInfo};
...
pub fn execute_mint(
    ...
    if info.sender != owner {
        ...
        // Block minting if wallet already has 3 NFTs
    }
...
pub fn execute_withdraw(
    ...
    let valid_treasury_account = deps.api.addr_validate(&config.treasury_account)?;

    Ok(Response::new().add_message(BankMsg::Send {
        amount, // Do we need to check that this is enough?
        to_address: valid_treasury_account.to_string(),
    }))
}
